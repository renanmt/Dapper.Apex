...
namespace Dapper.Apex
{
    public static partial class DapperApex
    {
        ...
        public static async Task<bool> ExistsAsync<T>(this IDbConnection connection, object key, 
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var exists = Convert.ToBoolean(await connection.ExecuteScalarAsync(sql, dynParams, transaction, commandTimeout));
            return exists;
        }
        ...
        /// <param name="commandTimeout">The operation timeout in milliseconds.</param>
        /// <returns>True if the entity exists in the database.</returns>
        public static async Task<bool> ExistsAsync<T>(this IDbConnection connection, T entity, 
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            var typeInfo = TypeHelper.GetTypeInfo(type);
            ...
            var exists = Convert.ToBoolean(await connection.ExecuteScalarAsync(sql, entity, transaction, commandTimeout));
            return exists;
        }
    }
}
