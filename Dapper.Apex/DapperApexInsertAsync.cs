...
namespace Dapper.Apex
{
    public static partial class DapperApex
    {
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            {
                using (var reader = await connection.QueryMultipleAsync(sql, entity, transaction, commandTimeout))
                {
                    var res = reader.Read();
                    var id = res.First().id;
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (typeInfo.KeyType == KeyType.Surrogate)
            {
                using (var reader = await connection.QueryMultipleAsync(sql, dynParams, transaction, commandTimeout))
                {
                    var keyProperty = typeInfo.PrimaryKeyProperties.First();
